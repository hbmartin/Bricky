...
enum RealityKitInstructionAdapter {
    static func makeEntity(
        ...
        for buffer in snapshot.buffers where !buffer.positions.isEmpty {
            ...
            let base = LDrawPalette.color(buffer.colorCode)
            let opacity: CGFloat = dimmed && !highlightedColors.contains(buffer.colorCode) ? 0.28 : 1
            let material = SimpleMaterial(color: base.withAlphaComponent(opacity), roughness: 0.28, isMetallic: false)
