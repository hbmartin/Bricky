...
import { forgeMeshFromText as forgeTripoMesh, type TripoConfig } from '../tripo.js';
...
export async function forgeMeshFromText(
  ...
  try {
    ...
    // Hosted Tripo text→3D.
    const config: TripoConfig = { apiKey: requireEnv('TRIPO_API_KEY') };
    const result = await forgeTripoMesh(prompt, size, config);
